...
function openPage(height, width) {
// KAT-END
...
// KAT-BEGIN save last window size
function openPageWithSavedSize() {
    chrome.storage.local.get('window', function(result) {
        var height = 730;
        var width = 750;
        if (result) {
            try {
                result = JSON.parse(result.window);
        ...
        openPage(height, width);
    });
}
// KAT-END

browser.browserAction.onClicked.addListener(openPageWithSavedSize);
