...
window.onload = function() {
    ...
    recordButton.addEventListener("click", function(){
        ...
        if (isRecording) {
            // Begin focus on window when recording
            if (contentWindowId) {
            ...
            // End
            notificationCount = 0;
            recordButton.childNodes[1].childNodes[2].textContent = " Stop Test";
            recordButton.childNodes[1].childNodes[1].src = './panel/images/stop-icon.png';
            $(recordButton.childNodes[1].childNodes[1]).addClass('stop-btn-pulse');
        }
        else {
            recordButton.childNodes[1].childNodes[2].textContent = " Record Test";
            recordButton.childNodes[1].childNodes[1].src = './panel/images/record-icon.png';
            $(recordButton.childNodes[1].childNodes[1]).removeClass('stop-btn-pulse');
        }
    })
...
function play() {
    // Begin content script not run on new tab
    ...
    .then(function(tabs) {
        // Begin focus on window when playing
        browser.windows.update(
            windowId,
            { focused: true }
        );
        ...
        if (!url || (!url.startsWith('http://') && !url.startsWith('https://'))) {
            sideex_log.info("Cannot execute directly from special pages like new tab page. Redirecting to https://www.google.com to initiate.");
            browser.tabs.update(tabId, {url: 'https://www.google.com'})
            .then(function() {
                if (checkPlayableInterval) {
                    clearInterval(checkPlayableInterval);
                    checkPlayableInterval = null;
                }
                checkPlayableInterval = setInterval(
                    function() {
...
function playDisable(setting) {
    if (setting) {
        document.getElementById("record").childNodes[1].childNodes[2].textContent = " Record";
        document.getElementById("record").childNodes[1].childNodes[1].src = '/panel/images/record-icon.png';
        $(document.getElementById("record").childNodes[1].childNodes[1]).removeClass('stop-btn-pulse');
    }
