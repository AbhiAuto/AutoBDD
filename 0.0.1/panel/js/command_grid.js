...
function attachEvent(start, end) {
    for (var i = start; i <= end; ++i) {
        ...
        node.addEventListener("click", function(event) {
            ...
            // Begin remove grid context menu & add Selenium commands
            addExecuteCommand(getCommandName(ref));
            // End

            // Begin show reference
            var command = new Command(getCommandName(ref));
...
// Begin Selenium commands
function addExecuteCommand(command) {
    ...
    if (command && command.length > 0) {
        ...
        a.innerHTML = "Execute";
        exe.appendChild(a);
        var tempId = $('.selectedRecord').attr('id');
